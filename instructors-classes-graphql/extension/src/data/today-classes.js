...
export async function todayClassesGraphQlQuery({ queryKeys, queryParameters }) {
    ...
    try {
        ...
        const sectionInstructors = result?.data?.sectionInstructors?.edges || [];

        const sections = [];
        if (sectionInstructors) {
            const dataSections = sectionInstructors.map( sectionInstructor => sectionInstructor.node.sections );
